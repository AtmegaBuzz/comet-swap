do
   local _ENV = _ENV
   package.preload["wusdc_bridge.utils.bint"] = function(...)
      local arg = _G.arg;
      ...
      end; local assert = _tl_compat and _tl_compat.assert or assert; local debug = _tl_compat and _tl_compat.debug or
          debug; local math = _tl_compat and _tl_compat.math or math; local _tl_math_maxinteger = math.maxinteger or
          math.pow(2, 53); local _tl_math_mininteger = math.mininteger or -math.pow(2, 53) - 1; local string = _tl_compat and
          _tl_compat.string or string; local table = _tl_compat and _tl_compat.table or table; local _tl_table_unpack =
          unpack or table.unpack; BigInteger = {}
      ...
      local function newmodule(bits, wordbits)
         ...
         function bint.isbint(x)
            return getmetatable(x) == bint_metatable
         end

         local function bint_assert_convert(x)
            if not bint.isbint(x) then
               print(debug.traceback())
               assert(bint.isbint(x),
                  'bint_assert_convert: expected BigInteger, got ' .. type(x) .. ' value ' .. tostring(x))
            end
            return x
         end
         ...
         local function bint_assert_convert_clone(x)
            if not bint.isbint(x) then
               print(debug.traceback())
               assert(bint.isbint(x),
                  'bint_assert_convert_clone: expected BigInteger, got ' .. type(x) .. ' value ' .. tostring(x))
            end
            local n = setmetatable({}, bint)
            local xi = x
            for i = 1, BINT_SIZE do
         ...
         local function bint_assert_convert_from_integer(x)
            ...
            assert(xi,
               'bint_assert_convert_from_integer: could not convert integer to big integer' ..
               type(x) .. ' value ' .. tostring(x))
            return xi
         end
         ...
         function bint.new(x)
            if getmetatable(x) ~= bint_metatable then
               local ty = type(x)
               if ty == 'number' then
         ...
         function bint.isintegral(x)
            return getmetatable(x) == bint_metatable or math_type(x) == 'integer'
         end

         function bint.isnumeric(x)
            return getmetatable(x) == bint_metatable or type(x) == 'number'
         end
...
do
   local _ENV = _ENV
   package.preload["wusdc_bridge.utils.tl-utils"] = function(...)
      local arg = _G.arg;
      local _tl_compat; if (tonumber((_VERSION or ''):match('[%d.]*$')) or 0) < 5.3 then
         local p, m = pcall(require, 'compat53.module'); if p then _tl_compat = m end
      end; local ipairs = _tl_compat and _tl_compat.ipairs or ipairs; local pairs = _tl_compat and _tl_compat.pairs or
          pairs; local table = _tl_compat and _tl_compat.table or table
...
local _tl_compat; if (tonumber((_VERSION or ''):match('[%d.]*$')) or 0) < 5.3 then
   local p, m = pcall(require, 'compat53.module'); if p then _tl_compat = m end
end; local assert = _tl_compat and _tl_compat.assert or assert; local pairs = _tl_compat and _tl_compat.pairs or pairs; local string =
    _tl_compat and _tl_compat.string or string; local bint = require('wusdc_bridge.utils.bint')(256, 32)
...
Name = Name or "Astro USD"
Ticker = Ticker or 'USDA'
Logo = Logo or 'seXozJrsP0OgI0gvAnr8zmfxiHHb5iSlI9wMI8SdamE-KY'
...
local function burnHandler(msg)
   assert(type(msg.Tags.Quantity) == 'string', 'Quantity is required!')
   assert(bint(msg.Tags.Quantity):lt(bint(Balances[msg.From])),
